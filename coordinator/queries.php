...
class Queries {
    ...
    public function update_college($conn,$id,$name,$city) {
        $sql = "UPDATE info_college SET collegeName=:name,city=:city WHERE collegeId=:id";
        $run = $conn->prepare($sql);
        $run->bindParam(':id', $id);
	...
	public function loginfaculty($conn,$email,$pass) {
        ...
        if(empty($email) || empty($pass)){
        	echo "<script>swal('Empty','Please Enter some details and continue', 'warning')</script>";
        	return '0';
        }
        if($p==md5($pass)){
        	session_start();
	...
	public function get_colleges($conn) {
		$sql = "SELECT * FROM info_college ORDER BY collegeName";
		$run = $conn->prepare($sql);
		$run->execute();
        return $run;
	}
	...
	public function get_subjects_by_branch($conn,$branch) {
		$sql = "SELECT * FROM info_subject WHERE branchId=:branch";
		$run = $conn->prepare($sql);
    ...
    public function get_subjects_by_branch_sem($conn, $bid, $sem) {
        $sql = "SELECT * FROM info_subject WHERE branchId=:branch AND semester=:sem";
        $run = $conn->prepare($sql);
    ...
    public function get_subject_list_faculty_all($conn, $facultyid, $semester) {
        if($semester == 0)
            $sql = $conn->prepare("SELECT * FROM info_subject 
                        WHERE subjectId IN(
                            SELECT subjectId FROM subjects WHERE userId = :userId )");
        else    
            $sql = $conn->prepare("SELECT * FROM info_subject 
                        WHERE semester = :semester AND subjectId IN(
                            SELECT subjectId FROM subjects WHERE userId = :userId )");
...
// end  
    ...
    public function get_subjects_by_id($conn,$id) {
        $sql = "SELECT * FROM info_subject WHERE subjectId=:subject";
        $run = $conn->prepare($sql);
    ...
    public function add_college($conn, $cid, $cname, $city) {
        $sql = $conn->prepare("INSERT INTO info_college (collegeId, collegeName, city) VALUES (:cid, :cname, :city)");
        $sql->bindParam(':cid', $cid);

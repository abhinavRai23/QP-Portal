...
if(isset($_POST['delfile'])) 
{
	...
	if ($delete_q) {
		if(unlink($filename))
		echo '<script type="text/javascript">swal("File successfully deleted");
...
<table align="center" style="width:40%">
	...
	if(isset($_POST['submit'])) {
		...
		$w = $obj->view_pdf_branch($conn, $branchId);
		$num_rows1 = $w->rowCount();
		if($num_rows1 != 0) {
		...
		while($d = $s->fetch()){
			...
			if($num_rows != 0){
				$flag++;
				echo "<center><h4><span><b >Subject Name:</b> $subjectName ($subjectId)</span></h4></center><br>";
				echo '<div>
			...
			<thead><tr>
				...
				while ($row = $query->fetch()) {
					...
					echo "<td><form method='POST' action='view_uploadedpdf.php'><input type='hidden' name='fileid' value='$fileid'><button type='submit' name='delfile' class=' btn btn-danger'><i class='fa fa-times'></i></button></form></td>";
